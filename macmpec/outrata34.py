...
model.f = Objective(expr=( (model.x[1] - 3)**2 + (model.x[2] - 4)**2 + (model.x[3] - 1)**2 + (model.x[4] - 1)**2 + model.y**2)/2)
...
model.nlcs2 = Complementarity(expr=complements(0 <= (1 + 0.1*model.y)*model.x[2] - model.y + model.x[3] + 2*model.x[2]*model.x[4], model.x[2] >= 0))
