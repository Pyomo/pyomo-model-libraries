...
# design-cent-1.py    QOR-AY-NLP-12-9-3
#
...
# ... sets
model.I = RangeSet(1,3)     # ... design variables
...
model.volume = Objective(expr=model.pi*model.x[3]**2, sense=maximize)
...
model.KKT_11 = Constraint(expr=1              + 2*(model.y[1,1] - model.x[1]) * model.l[1] == 0)
model.KKT_21 = Constraint(expr=2*model.y[2,1] + 2*(model.y[2,1] - model.x[2]) * model.l[1] == 0)

model.KKT_12 = Constraint(expr=-1/4           + 2*(model.y[1,2] - model.x[1]) * model.l[2] == 0)
model.KKT_22 = Constraint(expr=-1             + 2*(model.y[2,2] - model.x[2]) * model.l[2] == 0)

model.KKT_13 = Constraint(expr=0              + 2*(model.y[1,3] - model.x[1]) * model.l[3] == 0)
model.KKT_23 = Constraint(expr=1              + 2*(model.y[2,3] - model.x[2]) * model.l[3] == 0)

# ... complementarity & dual feasibility for lower level problem
def compl_(model, k):
    return complements(0 <= model.l[k], 
                       (model.y[1,k] - model.x[1])**2 + (model.y[2,k] - model.x[2])**2 - model.x[3]**2 <= 0)
