...
# design-cent-21.py    QOR-MY-NLP-13-13-3
#
...
model.KKT_12 = Constraint(expr=-1/4           + 2*(model.y[1,2] - model.x[1])/(model.x[3]**2) * l[2] == 0)
model.KKT_22 = Constraint(expr=-1             + 2*(model.y[2,2] - model.x[2])/(model.x[4]**2) * l[2] == 0)
...
def compl_(model, k):
    return complements(0 <= model.l[k], 
                       ((model.y[1,k] - model.x[1])**2)*(model.x[4]**2) + 
                        ((model.y[2,k] - model.x[2])**2)*(model.x[3]**2) <= (model.x[4]**2)*(model.x[3]**2))
