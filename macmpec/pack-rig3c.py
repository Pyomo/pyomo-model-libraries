...
#
# A packaging problem (see Section 9.2, Example 9.2):
# Minimize membrane surface under the condition that 
# the membrane comes into contact with a rigid obstacle. 
#
# **********************************************************
# ***** This model corrects a mistake in pack-rig2.mod *****
# ***** following a suggestion by Michal Kocvara.      *****
# **********************************************************
#
# Here added convexity constraint on the shape of the region
# defined by \alpha (see conv below).
#
...
model.a = Var(model.a_index, bounds=(0.6, 2.0), initialize=1.0)
...
def area_(model):
    return model.h/2 * sum(model.a[i] + model.a[i-1] for i in model.n)
...
def PDE_(model, i):
    return model.s1[i] == model.Au[i] - 1.1*model.l[i]
