...
#
# PENALTY FORMULATION THAT REMOVES FIX_MEM FROM ORIGINAL.
#
...
model.r = Param(initialize=10**6)                       # penalty parameter
...
# ... obstacle 
def xi_(model, i):
    return - 0.04*(model.x[i]**2 + (model.y[i]**2 - 0.25)**2)
...
def penalty_(model):
    return model.h/2 * sum(model.a[i] + model.a[i-1] for i in sequence(1,model.n)) + \
            model.r * model.h**2 * sum(model.u[i] - model.xi[i] - model.c*(model.l[i] - model.Au[i]) for i in model.Omega0)
model.penalty = Objective(rule=penalty_)
