...
def C_rule(model,i):
    if i == 0:
        ...
        return (-1)**i*model.B[K]/(model.B[i]*model.B[K-i])
...
model.x = Var(model.R,initialize=0.0)

def f_rule(model):
    return sum(-(cos(4*pi*model.T[i])+0.1*sin(i))*model.x[i] for i in model.R) - sum(0.5*(cos(4*pi*model.T[i])+0.1*sin(i))**2 for i in model.R) + sum(0.5*model.x[i]**2 for i in model.R)
