...
def f_rule(model):
	...
	sum((abs(1-model.x[i-1]-2*model.x[i+1]+(3-2*model.x[i])*model.x[i]))**(7/3.0) for i in xrange(2,N))+\
	(abs(-model.x[N-1]+1 +(3-2*model.x[N])*model.x[N]))**(7/3.0) +\
	sum((abs(model.x[i]+model.x[i+N/2]))**(7/3.0) for i in xrange(1,int(N/2)+1))
