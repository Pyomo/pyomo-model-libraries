...
def f_rule(model):
    return sum(-(cos(pi*model.T[i])+0.1*sin(i))*model.x[i] for i in model.R) + sum(0.5*(cos(pi*model.T[i])+0.1*sin(i))**2 for i in model.R) + sum(0.5*model.x[i]**2 for i in model.R)
