...
def create_model(busfile, branchfile):
    
    bus = []
    busmap = {}
    busfile = open(busfile, "r")
    ...
    def Gself(k):
        return bus[k].g_shunt + quicksum(branch[i].g*model.branch_tap[i]**2 for i in out_lines[k]) + quicksum(branch[i].g for i in in_lines[k])
    ...
    def Bself(k):
        return model.bus_b_shunt[k] + quicksum(branch[i].b*model.branch_tap[i]**2 + branch[i].c/2 for i in out_lines[k]) + quicksum(branch[i].b + branch[i].c/2 for i in in_lines[k])
    ...
    model.obj = Objective(expr = quicksum( \
        (bus[k].p_load + \
            quicksum( model.bus_voltage[k]*model.bus_voltage[branch[i].frm] * \
                 (Gin(i) * cos(model.bus_angle[k] - model.bus_angle[branch[i].frm]) + \
                  Bin(i) * sin(model.bus_angle[k] - model.bus_angle[branch[i].frm])) \
                 for i in in_lines[k]) + \
            quicksum( model.bus_voltage[k]*model.bus_voltage[branch[i].to] * \
